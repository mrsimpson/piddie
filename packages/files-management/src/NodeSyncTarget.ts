...
export class NodeSyncTarget implements SyncTarget {
    ...
    constructor(public readonly id: string, private rootDir: string) { }
            ...
            try {
                switch (change.type) {
                    case "create":
                        ...
                        const exists = await this.fileSystem.exists(change.path);
                        if (exists) {
                            // File exists, check for conflict
                            const existingContent = await this.fileSystem.readFile(change.path);
