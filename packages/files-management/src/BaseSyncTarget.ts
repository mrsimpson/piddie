...
export abstract class BaseSyncTarget implements SyncTarget {
    ...
    protected async doSync(metadata: FileMetadata, contentStream: FileContentStream): Promise<FileConflict | null> {
        if (!this.fileSystem) {
            throw new SyncOperationError("Not initialized", "INITIALIZATION_FAILED");
        }
    ...
    abstract initialize(fileSystem: FileSystem, isPrimary: boolean): Promise<void>;

    async notifyIncomingChanges(paths: string[]): Promise<void> {
        await this.doCollect(paths);
    }

    async applyFileChange(metadata: FileMetadata, contentStream: FileContentStream): Promise<FileConflict | null> {
