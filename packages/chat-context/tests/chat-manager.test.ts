...
import { DexieChatManager, ChatDatabase } from '../src/chat-manager'
...
const chatMocks = {
  ...
  update: vi.fn<(id: string, changes: Partial<Omit<Chat, 'messages'>>) => Promise<number>>(),
  ...
  describe('GIVEN a new chat manager', () => {
    ...
    describe('WHEN updating message status', () => {
      ...
      it('THEN should throw error for non-existent message', async () => {
        ...
        await expect(
          chatManager.updateMessageStatus('chat_1', 'non-existent', MessageStatus.SENT)
        ).rejects.toThrow('Message not found')
      })
    })

    describe('WHEN listing chats', () => {
      const createMockChat = (id: string, lastUpdated: Date): Omit<Chat, 'messages'> => ({
        id,
    ...
    describe('WHEN deleting a chat', () => {
      ...
      it('THEN should throw error for non-existent chat', async () => {
        chatMocks.get.mockResolvedValueOnce(undefined)
        await expect(chatManager.deleteChat('non-existent')).rejects.toThrow('Chat not found')
      })
    })
  })
})
