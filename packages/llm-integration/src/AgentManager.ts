...
export class AgentManager {
  ...
  public async initializeFromStoredSettings(chatId: string): Promise<void> {
    try {
      ...
      console.log(
        `[AgentManager] Initialized agent for chat ${chatId} from stored settings:`,
        storedSettings
      );
    } catch (error) {
      console.error(
        `[AgentManager] Error initializing agent for chat ${chatId}:`,
        error
      );
    }
  }
  ...
  async configureAgent(
    ...
    try {
      ...
      console.log(
        `[AgentManager] Agent configured for chat ${chatId}:`,
        existingContext.config
      );
    } catch (error) {
      console.error(
        `[AgentManager] Error persisting agent settings for chat ${chatId}:`,
        error
      );
    }
  }
  ...
  async react(
    ...
    if (context.config.autoContinue) {
      const systemMessage =
        this.formatToolCallsForSystemMessage(completedToolCalls);
      return {
        type: "continue",
